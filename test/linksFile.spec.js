...
test('Deberia retornar un array de objetos, getLinks', (done) => {
    expect(getLinks('##Bienvenida a tu primer proyecto del track de Frontend en Laboratoria [LABORATORIA](https://www.laboratoria.la/)', './folder/readmeTuto.md'))
        .toEqual([{
            href: 'https://www.laboratoria.la/',
            text: 'LABORATORIA',
            file: `./folder/readmeTuto.md`
        }])
    done()
});
test('deberia retornar una promesa que resuelve un array de objetos,promiseLinks', (done) => {
    return promiseLinks('./folder/readmeTuto.md').then(result => {
        expect(result).toEqual([{
            href: 'https://github.com/nodejs/node/blob/master/doc/topics/blocking-vs-non-blocking.md',
            text: 'LABORATORIA',
            file:
