...
describe('test de la libreria md-link003 en mejor de los casos',()=>{
    it('Debería retornar links en el archivo markdonw',(done)=>{
        return excute('md-links', ['./folder/readmeTuto.md'])
        .then(result =>{
        expect(result).toBe('ruta: ./folder/readmeTuto.md, link : https://github.com/nodejs/node/blob/master/doc/topics/blocking-vs-non-blocking.md, texto : LABORATORIA\n')
        done()
    })
    })
    it('Debería retornar links y validarlos',(done)=>{
        return excute('md-links', ['./folder/readmeTuto.md','--validate'])
        .then(result =>{
            expect(result).toBe('ruta: ./folder/readmeTuto.md, link : https://github.com/nodejs/node/blob/master/doc/topics/blocking-vs-non-blocking.md, fail, 404 , texto : LABORATORIA\n')
            done()
        })
    });
    it('Debería retornar estadistica de los link',()=>{
        return excute('md-links', ['./folder/readmeTuto.md','--stats'])
        .then(result =>{
            expect(result).toBe('Total:1\nUnique:1\n')
        })
    });
    it('Debería retornar estadistica y validar los links',()=>{
        return excute('md-links', ['./folder/readmeTuto.md','--stats','--validate'])
        .then(result =>{
            expect(result).toBe('Total:1\nUnique:1\nBroken:1\n')
        })
    });
...
describe('Test de la libreria md-links003 en prueba de error',()=>{
    ...
    it('Debería mostrar de como usar la libreria',()=>{
        return excute('md-links',['./folder/readmeTuto.md','-cualesquierLetra'])
        .then(result =>expect(result).toBe('Usage md-links PATH [--stats] [--validate]\n'))
    });
    it('Debería mostrar de como usar la libreria',()=>{
        return excute('md-links',['./folder/readmeTuto.md','--stats','--otroTexto'])
        .then(result =>expect(result).toBe('Usage md-links PATH [--stats] [--validate]\n'))
    })
    it('Debería mostrar de como usar la libreria',()=>{
        return excute('md-links',['./folder/readmeTuto.md','--validate','--stats'])
        .then(result =>expect(result).toBe('Usage md-links PATH [--stats] [--validate]\n'))
    })
    it('Debería mostrar de como usar la libreria',()=>{
        return excute('md-links',['./folder/readmeTuto.md','--stats','--validate','otroTexto'])
        .then(result =>expect(result).toBe('Usage md-links PATH [--stats] [--validate]\n'))
    })
})
